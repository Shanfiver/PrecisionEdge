 ...
 * rather than an extraction from it. It reuses the same `pe-header` /
 * `pe-nav` class names and tokens, just on a solid white bar instead of a
 * transparent one.
 */
export default function Header() {
  ...
  return (
    <header className="pe-header pe-header--solid">
      <Link to="/" className="pe-header__logo" onClick={() => setMenuOpen(false)}>
      ...
      <nav className={`pe-nav${menuOpen ? ' is-open' : ''}`}>
        ...
        <Link to="/contact" className="pe-nav__cta" onClick={() => setMenuOpen(false)}>
          CONTACT US
        </Link>
      </nav>
    </header>
